...
import { emitToKitchen, emitToWaiters, emitToCashiers, emitToBranch } from '../config/socket';
...
export const addItems = async (
    ...
    try {
        ...
        const order = await Order.findById(req.params.id).populate('tableId', 'tableNumber branchId');
        ...
        if (order.status !== 'active') {
            throw ApiError.badRequest('Cannot add items to completed/cancelled order');
        }
...
export const updateItemStatus = async (
    ...
    try {
        ...
        emitToKitchen(order.branchId.toString(), 'item:status-changed', eventData);
